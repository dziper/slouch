...
while key != ESC:
    ...
    if(len(contAngles)>1):
        ...
        if (distFromCenter > slouchThresh):
            ...
            cv2.putText(frame, "Stop slouching!", (10,25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 3)
        else:
            startTime = None
        ...
        #can't find contours
        startTime = None
        cv2.putText(frame, "Can't find contours!", (10,25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 3)
    ...
    if cursorBGR[0] != -1:
        colorString = "R: " + str(cursorBGR[2]) + " G: " + str(cursorBGR[1]) + " B: " + str(cursorBGR[0])
